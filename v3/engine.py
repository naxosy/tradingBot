...
def check_for_new_trades(df, current_trades, account_status):
    ...
    if last_ema_5 < last_ema_13 and ema_5 > ema_13: # Signal for buy trade
        sl = last_ema_13
        tp = price + (price - sl) * 3
    ...
    if last_ema_5 > last_ema_13 and ema_5 < ema_13: # Signal for sell trade
        sl = last_ema_13
        tp = price + (price - sl) * 3
        size_in_EUR = account_status['balance']*config.RISK

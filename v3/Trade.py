...
class Trade:
    ...
    def __str__(self):
            if self.closed :
                ...
                return f"Trade({self.kind} {self.size:.1f}) fermé ({header}) à {self.close_date} : sortie à {self.exit_price:.5f} soit un profit de {self.profit:.1f} pips"
            else :
                tp = f"{self.tp:.5f}" if self.tp is not None else "N/A"
                sl = f"{self.sl:.5f}" if self.sl is not None else "N/A"
                return f"Trade({self.kind} {self.size:.1f}) ouvert à {self.open_date} : entrée à {self.entry_price:.5f} avec un TP à {tp} et un SL à {sl}"

...
def update_trades(df:pd.DataFrame, current_trades:List[Trade]) -> List[Trade]:
    updated_trade_list = []
    for trade in current_trades:
        trade.sl = df.iloc[-1]['ask']['ema_13']
        log.log_update_trade(df.index[-1], trade)
...
def process_new_trades(df:pd.DataFrame, current_trades:List[Trade]) -> List[Trade]:
    ...
    if last_ema_5 < last_ema_13 and ema_5 > ema_13: # BUY signal
        sl = last_ema_5
        tp = price + (price - sl)*3
        new_trade = Trade("BUY", config.SIZE, price, date, sl=sl, tp=tp)
    ...
    if last_ema_5 > last_ema_13 and ema_5 < ema_13 : # SELL signal
        sl = last_ema_5
        tp = price - (sl - price)*3
        new_trade = Trade("SELL", config.SIZE, price, date, sl=sl, tp=tp)

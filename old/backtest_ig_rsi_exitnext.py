...
# === Ne garder que les lignes exploitables ===
...
print(df[['timestamp', 'bid_Close', 'rsi', 'atr']].head(10))
print(f"Nombre de lignes avec RSI et ATR valides : {len(df)}")
...
for i in range(len(df) - 1):
    ...
    if rsi < 30 and last_trade_direction != "BUY":
        print(f"{row['timestamp']} | 📈 SIGNAL BUY : RSI={rsi:.2f}")
        direction = "BUY"
        stop_loss = entry_price - atr
        take_profit = entry_price + 0.8 * atr
    elif rsi > 70 and last_trade_direction != "SELL":
        print(f"{row['timestamp']} | 📉 SIGNAL SELL : RSI={rsi:.2f}")
        direction = "SELL"
        stop_loss = entry_price + atr
        take_profit = entry_price - 0.8 * atr
    else:
    ...
    if hit_tp:
        ...
        # Sortie forcée au prix suivant
        result = (exit_price - entry_price) * lot_size * pip_value if direction == "BUY" else (entry_price - exit_price) * lot_size * pip_value
...
trades_df.to_csv("backtest_ig_rsi_exitnext_results.csv", index=False)
print("✅ Backtest terminé. Résultats enregistrés dans 'backtest_ig_rsi_exitnext_results.csv'")

# === Statistiques globales ===
total_trades = len(trades_df)
winning_trades = trades_df[trades_df['result'] > 0].shape[0]
losing_trades = trades_df[trades_df['result'] < 0].shape[0]
